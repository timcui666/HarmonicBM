...
class ComplexUniDeepFsmn_L1(nn.Module):
    ...
    def forward(self, x):
        ...
        Args:
            x: torch with shape [batch, channel, feature, sequence, 2], eg: [6, 256, 1, 106, 2]
        """
        # torch.Size([1, 128, 159, 319, 2])
        b, c, h, T, d = x.size()
        # x : [b,T,h,c,2] torch.Size([1, 319, 159, 128, 2])
        x = torch.transpose(x, 1, 3)
        # torch.Size([319, 159, 128, 2])
        x = torch.reshape(x, (b * T, h, c, d))

        # 实数和虚数的操作：DCCRN，实数乘实数等于实数，虚数乘虚数等于-实数，实数乘虚数等于虚数。
        # torch.Size([319, 159, 128])
